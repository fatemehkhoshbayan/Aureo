...
from app.migration_utils import has_index, has_table
...
def upgrade() -> None:
    ...
    if not has_index(bind, "users", "ix_users_email"):
        op.create_index(op.f("ix_users_email"), "users", ["email"], unique=True)

    if not has_table(bind, "categories"):
        op.create_table(
            "categories",
    ...
    if not has_table(bind, "photographers"):
        op.create_table(
            "photographers",
    ...
    if not has_table(bind, "packages"):
        op.create_table(
            "packages",
            sa.Column("id", sa.String(length=50), nullable=False),
    ...
    if not has_table(bind, "portfolio_items"):
        op.create_table(
            "portfolio_items",
            sa.Column("id", sa.String(length=50), nullable=False),
            sa.Column("photographer_id", sa.String(length=50), nullable=False),
